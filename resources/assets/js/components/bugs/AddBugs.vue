<template>
    <modal name="add-bugs" transition="nice-modal-fade" @before-open="beforeOpen">
                ...
                <el-form :model="form" ref="form" label-position="top" v-loading="isProcessing" style="width: 100%">
                    <div class="buzz-modal-content">
                        <el-form-item prop="name">
                            <el-input type="text" v-model="form.name" placeholder="Title"></el-input>
                        </el-form-item>
                        <el-form-item>
                        ...
                        <el-form-item  class="form-buttons">
                            ...
                            <el-button @click="$modal.hide('add-bug')">Cancel</el-button>
                        </el-form-item>
                    </div>
                </el-form>
            </div>
        ...
        methods: {
            beforeOpen (event) {
                if(typeof event.params != 'undefined' && event.params.action == 'update') {
                    this.action = 'Update';
                    this.header = 'Edit Bugs';
                    this.id = event.params.data.id;
                    var vm = this;
                    axios.get('api/bugss/'+this.id)
                        .then( response => {
            ...
            save: function () {
                ...
                axios.post('/api/bugss/new',this.form)
                .then( response => {
                    this.isProcessing = false;
                    swal('Success!', 'Bugs is saved!', 'success');
                })
                .catch ( error => {
                    this.isProcessing = false;
            ...
            update: function () {
                axios.put('/api/bugss/'+this.id+'/edit', this.form)
                .then( response => {
                    this.isProcessing = false;
                    swal('Success!', 'Bugs is updated!', 'success');
                })
                .catch ( error => {
                    if(error.response.status == 422){
                    this.errors = error.response.data.errors;
