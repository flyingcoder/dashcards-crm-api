...
class GroupPolicy
{
    ...
    {
        if( !auth()->user()->hasRole('admin|manager') && auth()->user()->can('group.view') )
    ...
    {
       if( !auth()->user()->hasRole('admin|manager') && !auth()->user()->can('group.create') )
    ...
    {
        if(!auth()->user()->hasRole('admin') && !auth()->user()->can('group.update') )
          abort(403, 'Not enought permission!');
    }
    ...
    public function delete(Role $role)
    {
        ...
        if( !auth()->user()->hasRole('admin') && !auth()->user()->can('group.delete') )
            abort(403, 'Not enought permission!');
